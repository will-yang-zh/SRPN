...
df = pd.read_csv('images-Set3/labels.csv')

def get_file_name(filename):
    filename = 'images-Set3/'+filename
    file_name_images = xt.parse(filename).getroot().find('filename').text
    file_path_images = os.path.join('./images-Set3/Images', file_name_images)
...
tb = TensorBoard('object_detection_set3')
# history = model.fit(x=X_train, y=y_train, batch_size=32, epochs=400, validation_data=(X_test,y_test), callbacks=[tb])
history = model.fit(x=X_train, y=y_train, batch_size=10, epochs=150, validation_data=(X_test, y_test))

model.save('Models/Set3_object_detection_William_02_10_2021.h5')
